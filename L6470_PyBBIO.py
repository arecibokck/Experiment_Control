...
#case-sensitive declaration - defines mode 3
...
CMD_SetParam = 0b00000000
CMD_Move = 0b01000000
CMD_GetParam = 0b00100000
CMD_ResetDevice = 0b11000000
CMD_GetStatus = 0b11010000
...
def setup(self):
    ...
    for i in (self._cs, self._sdi, self._clk): pinMode(i, OUTPUT)

    digitalWrite(cs_pin, HIGH)
    pinMode(sdo_pin, INPUT)

    #BUSY/SYNC AND RESET pins must be added
...
def io(data_out):
    ...
    data_in = shiftOut(self._sdo, self._clk, msbfirst, int(data_out), FALLING)
    digitalWrite(cs_pin, HIGH)
...
def GetAcc():
    ...
    io(CMD_GetParam | REG_Acc)
    acc = io(bin(0))
    acc |= io(bin(0))
    return acc

def SetAcc(float degreePerSec2):
...
def GetMinSpeed():
    io(CMD_GetParam | REG_Min_Speed)
    min_speed = io(bin(0))
    min_speed |= io(bin(0))
    return min_speed;

def SetMinSpeed(degreePerSec):
    min_speed = MinSpdCalc(long(angleToSteps(degreePerSec)))
