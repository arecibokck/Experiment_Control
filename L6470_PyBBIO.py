...
class L6470:
    ...
    def __init__(self, spi = SPI0, cs=0):
        ...
        self._spi.begin()
        self._spi.setClockMode(3)#self._cs, 3) #Mode - 3
        self._spi.setMaxFrequency(2000000)#self._cs, 2000000) #2MHz
        self._spi.setBitsPerWord(8)#self._cs, 8)  #8 bits per word
        self._io(self.CMD_ResetDevice)
        self._io(self.CMD_SetParam | self.REG_StepMode)
        ...
        self._io(self.CMD_SetParam | self.REG_FSspeed)
        self._io(0x03ff, length=2) #Do not switch to full steps.


    def _io(self, data_out):

        return byteArrayToInt(self._spi.transfer(self._cs, intToByteArray(data_out, length)))
