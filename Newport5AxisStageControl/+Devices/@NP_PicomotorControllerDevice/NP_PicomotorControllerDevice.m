...
classdef NP_PicomotorControllerDevice < Devices.Device
    ...
    properties
        ...
        % USB or ETHERNET
        ConnectionType;
        % true if device ready to perform next command
        ReadyStatus = false;
        ...
        % #Iterations until 'WaitforStops' sends Timeout
        Pause_Max_Iterations =-1;
        % Time in [s] per wait-iteration
        Pause_Time=0.1;
    ...
    methods
        ...
        %% ConnectToDevice
        function ConnectToDevice(this, ConnectionType)
            ...
            assert(any(strcmpi(ConnectionType,{'USB','Ethernet'})),...
                'InputError:  ConnectionType must be "USB" or "Ethernet"')
            disp('Looking for Controller device(s)...')
            this.ConnectionType = upper(ConnectionType);
            try
                switch this.ConnectionType
                    ...
                    case 'ETHERNET'
                        ...
                        try
                            ErrorType = 0; % - cannot connect
                            fopen(this.NP_ETHERNET);
                            ErrorType = 1; % - mismatch in identificationquery of deviceKey and this.deviceKey
                            devInfo = this.query(this.CommandList.IdentificationQuery);
                            ...
                            assert( strcmpi(devInfo(end-4:end),extractAfter(this.deviceKey,'-')),...    %Dirty fix: Hostname is by default the device key but this could be changed!
                                'Error:  "this.deviceKey" does not match the answer from "*IDN?" ')
                            
                            disp('NP_PicomotorControllerDevice connected')
                        catch ME
                            warning(ME.message)
                            if ErrorType ==1
                                this.DisconnectFromDevice;
                            end
                            warning(['Could not connect to device with Key ' this.deviceKey])
                        end
                    otherwise
                        error(['No connection implemented for this.ConnectionType = ' ConnectionType])
                end
            catch ME
                warning(ME.message)
                rethrow(ME)
    ...
    methods
        ...
        % IsPicomotorMoving  (Check if an axis is moving)
        ...
        function Error = MoveIndefinitely(this, ChannelNumber, direction)
            ...
            if isequal(direction, '+')
                disp(['Moving Channel ' num2str(ChannelNumber) ' indefinitely in the positive direction...']);
            else
                disp(['Moving Channel ' num2str(ChannelNumber) ' indefinitely in the negative direction...']);
            end
    ...
    methods
        ...
        function Error = SetMotorType(this, ChannelNumber, motortype)
            ...
            % - check that ChannelNumber is valid
            ...
            isaninteger = @(x)isfinite(x) && x==floor(x);
            assert(isaninteger(ChannelNumber) && ChannelNumber > 0 && ChannelNumber < 5 ,'Invalid channel number. Check if it is an integer between 1 and 4.');
            % - check that Motortype is correct
